...
int testSplayIndex(int n, int size)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		if (root)
		{
			...
			index = rand() % size;
			leaf = tree_splayIndex(root, index);
	...
	errors += genReport(buildErrors,n,"tree_splayIndex(): build error");
	errors += genReport(splayErrors,n,"tree_splayIndex(): splay fail");
	errors += genReport(matchErrors,n,"tree_splayIndex(): splayed wrong node");
	if (matchErrors)
	{
	...
	errors += genReport(rootErrors,n,"tree_splayIndex(): root parent not updated");
	errors += genReport(leftSubTreeErrors,n,"tree_splayIndex(): bad left tree size");
	errors += genReport(rightSubTreeErrors,n,"tree_splayIndex(): bad right tree size");
